...
from . utils import get_available_cores
...
class MolecularReplacement:
    ...
    def run_phaser_molecular_replacement_async(
        self, params_filename, working_dir, ignore_timeout=False
    ):
        try:
            ...
            while phaser_process.poll() is None:
                ...
                if self.terminate_flag:
                    self.logger.warning(
                        "Terminating phaser run due to external request"
                    )
                ...
                if os.path.exists(phaser_log_path):
                    ...
                    if log_size_mb > 4:
                        self.logger.warning(
                            "Terminating phaser run due to log file size exceeding 4MB."
                        )
                        phaser_process.terminate()
                        break
                    ...
                    if not found_good_tfz:
                        ...
                        try:
                            with open(phaser_log_path, "rb") as log_file:
                                ...
                                text_content = bytes_content.decode(
                                    "utf-8", errors="ignore"
                                )
                            ...
                            if "SOLU 6DIM ENSE" in line:
                                ...
                                if match:
                                    ...
                                    if tfz_value >= 8.0:
                                        found_good_tfz = True
                                        self.logger.info(
                                            f"Found TFZ value {tfz_value} >= 8.0. Allowing process to continue."
                                        )
                                        break
                        if not found_good_tfz:
                            ...
                            if not ignore_timeout:
                                ...
                                if elapsed_time > timeout_seconds:
                                    self.logger.warning(
                                        f"Terminating phaser run due to exceeding the {timeout_seconds} seconds time limit."
                                    )
                                    phaser_process.terminate()
                                    break
                        else:
    ...
    def handle_phaser_output(self, output_dir, ensemble_pdbs=None):
        try:
            ...
            if all(
                os.path.exists(file)
                for file in ["PHASER.log", "PHASER.1.pdb", "PHASER.1.mtz"]
            ) or all(
                ...
                if self.is_phaser_successful(output_dir):
                    logging.info(
                        f"Phaser molecular replacement was successful. Check log file for details at {output_dir}/PHASER.log"
                    )
                    ...
                    if (
                        tfz is not None
                        and float(tfz) >= 8.0
                        and float(llg) >= llg_threshold
                    ):
                        success = True
                        if ensemble_pdbs is None:
                            logging.info(
                                f"Phaser molecular replacement with input model was successful. Check log file for details at {output_dir}/PHASER.log"
                            )
                        else:
                            logging.success(
                                f"Phaser molecular replacement alternative mode with ensemble models {ensemble_pdbs} was successful. Check log file for details at {output_dir}/PHASER.log"
                            )
                    elif (
                        tfz is not None
                        and float(tfz) >= 8.0
                        ...
                        for file in itertools.chain(
                            glob.glob(f"{output_dir}/PHASER.*"),
                            glob.glob(f"{output_dir}/*.params"),
                        ):
                            name_base = os.path.basename(file)
                            shutil.copy(file, f"{save_dir}/{name_base}")
                        logging.warning(
                            f"Phaser molecular replacement has low llg ({llg}) but with acceptable tfz {tfz} and may not be reliable. Check log file for details at {save_dir}/PHASER.log"
                        )
                    elif (
                        tfz is not None
                        and float(tfz) >= 8.0
                        and "** SINGLE solution" in content
                        and int(llg) == 0
                    ) or (
                        tfz is not None
                        and float(tfz) >= 8.0
                        and (int(llg) == 0 or float(llg) == 0.0)
                    ):
                        # move the files into a save folder
                        save_dir = os.path.join(output_dir, "save")
                        os.makedirs(save_dir, exist_ok=True)
                        for file in glob.glob(f"{output_dir}/PHASER.*"):
                            ...
                            shutil.copy(file, f"{save_dir}/{name_base}")
                        logging.success(
                            f"Phaser molecular replacement may have TNCS present but with acceptable tfz {tfz}. Check log file for details at {save_dir}/PHASER.log"
                        )
                        success = True
                    elif (
                        tfz is not None
                        and 6.0 <= float(tfz) < 8.0
                        and "** SINGLE solution" in content
                    ):
                        save_dir = os.path.join(output_dir, "save")
                        os.makedirs(save_dir, exist_ok=True)
                        for file in glob.glob(f"{output_dir}/PHASER.*"):
                            ...
                            shutil.copy(file, f"{save_dir}/{name_base}")
                        logging.warning(
                            f"Phaser molecular replacement has moderate tfz ({tfz}) with a single solution. Check log file for details at {save_dir}/PHASER.log.\nTesting a refinement with the current solution."
                        )
                        success = True

                    else:
                        logging.warning(
                            f"Phaser molecular replacement has low tfz ({tfz}) and may not be reliable. Check log file for details at {output_dir}/PHASER.log"
                        )
                else:
                    logging.warning(
                        f"Phaser molecular replacement may have failed. Check log file for details at {output_dir}/PHASER.log"
                    )
            else:
                for file in glob.glob("PHASER.*"):
                    shutil.move(file, f"{output_dir}/{file}")
                for file in glob.glob("alternative_phaser.params"):
                    shutil.copy(file, f"{output_dir}/{file}")
                logging.warning(
                    f"Phaser molecular replacement may have failed because not all output files are present. Check log file for details at {output_dir}/PHASER.log"
                )
                            ...
                            for sol_line in lines[lines.index(line) :]:
                                ...
                                if in_solu_set:
                                    ...
                                    if tncs_present and "TFZ" in sol_line:
                                        tfz_values = [
                                            float(part.split("=")[-1])
                                            for part in sol_line.split()
                                            if "TFZ=" in part
                                        ]
                                        highest_tfz = max(
                                            highest_tfz, max(tfz_values, default=0.0)
                                        )

                                    if "LLG" in sol_line and "TFZ" in sol_line:
                                        llg_tfz_pairs_found = re.findall(
                                            r"LLG=(\d+) TFZ==(\d+\.\d+)", sol_line
                                        )
                                    ...
                                    if "TFZ" in tfz_part:
                                        ...
                                        if tfz > highest_tfz:
                                            ...
                                            for llg, tfz_scores in llg_tfz_pairs:
                                                if (
                                                    abs(float(tfz_scores) - highest_tfz)
                                                    < tolerance
                                                ):
                                                    highest_llg_tfz_pair = (
                                                        llg,
                                                        tfz_scores,
                                                    )
    ...
    def multi_analyze_asu_and_solvent_content(
        self, mtz_file, csv_file, sequence_ids, specified_copy_numbers=None
    ):
        """
        ...
        def get_mean_matthews_coeff(space_group_number):
            ...
            return space_groups.get(
                space_group_number, 2.69
            )  # Default to general mean if not found
        ...
        def estimate_amino_acid_range(mtz_file):
            ...
            mw_residue = (
                110  # Average molecular weight of an amino acid residue in Daltons
            )

            asu_volume = (
                crystal_symmetry.unit_cell().volume()
                / crystal_symmetry.space_group().order_z()
            )
        ...
        def find_combinations(proteins, mtz_file, min_residues, max_residues):
            def attempt_find_combinations(copy_ranges):
                ...
                for copy_nums in itertools.product(
                    *[range(*copy_ranges[protein]) for protein in proteins]
                ):
                    sorted_combo = sorted(
                        [
                            (protein, copy_nums[i], proteins[protein])
                    ...
                    if min_residues <= total_residues <= max_residues:
                        ...
                        combo_str = ", ".join(
                            [f"{chain[0]} x {chain[1]}" for chain in sorted_combo]
                        )
                        combinations.append(
                            (combo_str, prob, solvent_content, matthews_coeff)
                        )
            ...
            copy_ranges = {
                protein: (1, 5) if length > 50 else (1, 3)
            ...
            if not combinations:
                expanded_copy_ranges = {
                    protein: (1, 10) if length > 50 else (1, 5)
                    for protein, length in proteins.items()
                }
                combinations, closest_combination = attempt_find_combinations(
                    expanded_copy_ranges
                )

            # Handle the case when protein length is less than min_residues
            if not combinations and all(
                length < min_residues for length in proteins.values()
            ):
                logging.warning(
                    "Warning: Protein length less than estimated minimum. Checking for 1 copy, potential other components like nucleic acids may be present."
                )
                for protein, length in proteins.items():
                    if length < min_residues:
                        matthews_rupp_result = matthews_rupp(
                            crystal_symmetry=crystal_symmetry, n_residues=length
                        )
                ...
                closest_combination = (
                    min(combinations, key=lambda x: abs(x[3] - mean_matthews_coeff))
                    if combinations
                    else None
                )
        ...
        min_residues, max_residues, asu_volume, crystal_symmetry = (
            estimate_amino_acid_range(mtz_file)
        )
        mean_matthews_coeff, combinations, closest_combination = find_combinations(
            proteins, mtz_file, min_residues, max_residues
        )
        ...
        if specified_copy_numbers:
            ...
            if len(sequence_ids) != len(specified_copy_numbers):
                raise ValueError(
                    "Length of specified copy numbers does not match the number of sequences."
                )

            # Calculate Matthews coefficient and solvent content directly from specified copy numbers
            total_residues = sum(
                proteins[sequence_id] * specified_copy_numbers[sequence_id]
            ...
            combo_str = ", ".join(
                [
                    f"{protein_id} x {copy_num}"
                    for protein_id, copy_num in specified_copy_numbers.items()
                ]
            )
            specified_combination = (combo_str, None, solvent_content, matthews_coeff)
            copy_numbers_all_combinations[combo_str] = (
                None,
                solvent_content,
                matthews_coeff,
            ...
            print(
                f"Probability: {closest_combination[1]:.2%}, Solvent Content: {closest_combination[2]:.2f}, Matthews Coefficient: {closest_combination[3]:.2f}"
            )
    ...
    def generate_phaser_params_multimer(
        self,
        params_filename,
        hklin,
        solvent_content,
        ...
        try:
            ...
            print(
                f"Warning: Could not get available cores: {e}. Using provided nproc value."
            )
            final_nproc = nproc
    ...
    def deduce_missing_copies(
        ...
        for combo_str, (
            ...
            matthews_coeff,
            copy_numbers,
        ) in all_combinations.items():
            if all(
                found_copies[protein_id] <= copies
                for protein_id, copies in copy_numbers.items()
            ):
                filtered_combinations.append(
                    (
                        ...
                        solvent_content,
                        matthews_coeff,
                        abs(matthews_coeff - mean_matthews_coeff),
                    )
                )
        ...
        if len(filtered_combinations) > 1 and top_switch:
            return (
                filtered_combinations[1][1],
                filtered_combinations[1][3],
                found_copies,
            )  # Return the copy numbers of the second closest combination
        elif len(filtered_combinations) > 1 and not top_switch:
            return (
                filtered_combinations[0][1],
                filtered_combinations[0][3],
                found_copies,
            )  # Return the copy numbers of the closest combination
        else:
            return (
                None,
                None,
                found_copies,
            )  # Return found_copies even if no suitable second combination

    def generate_phaser_params_for_second_run(
        self,
        params_filename,
        hklin,
    ...
    def find_autobuild_inputs(self, output_root):
        ...
        if latest_phaser_mtz:
            ...
            partial_pdbs = glob.glob(
                os.path.join(latest_phaser_pdb_dir, "*_partial.pdb")
            )

            if partial_pdbs:
                latest_phaser_pdb = partial_pdbs[
                    0
                ]  # Select the first *_partial.pdb if available
            else:
