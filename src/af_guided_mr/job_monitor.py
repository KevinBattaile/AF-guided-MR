...
class JobMonitor:
    ...
    def monitor_and_resolve_hangs(self, autobuild_log_path, autobuild_process):
        self._monitor(
            autobuild_log_path, autobuild_process, self.check_and_resolve_hangs
        )

    def monitor_and_resolve_memory_leaks(self, autobuild_log_path, autobuild_process):
        self._monitor(
            autobuild_log_path, autobuild_process, self.check_and_resolve_memory_leaks
        )
    ...
    def check_and_resolve_memory_leaks(self, autobuild_log_path):
        info_file_paths, missing_info_file_paths, job_paths, missing_job_paths = (
            self._extract_info_and_job_paths(autobuild_log_path)
        )
        if info_file_paths:
        ...
        if missing_job_paths:
            ...
            for missing_job_path in missing_job_paths:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error creating STOPWIZARD file in missing job path {missing_job_path}: {e}"
                    )
                go_on_file = os.path.dirname(autobuild_log_path) + "/GO_ON"
                list_of_jobs_running_file = (
                    os.path.dirname(autobuild_log_path) + "/LIST_OF_JOBS_RUNNING"
                )
                try:
                    ...
                    with open(list_of_jobs_running_file, "r") as file:
                        ...
                        if final_count is not None:
                            ...
                            self.logger.warning(
                                "Final count not found in LIST_OF_JOBS_RUNNING file."
                            )
                except Exception as e:
                    self.logger.error(
                        f"Error creating GO_ON file in missing job path {missing_job_path}: {e}"
                    )
    ...
    def _parse_log_for_job_paths(self, log_contents):
        job_logs = re.findall(
            r"Log will be: (\/[\w\/.-]+\/RUN_FILE_(\d+)\.log)", log_contents
        )
        job_paths = []
    ...
    def _resolve_job_block(self, job_paths, resolve_hangs=False):
        finished_jobs = [
            job for job in job_paths if os.path.exists(os.path.join(job, "FINISHED"))
        ]
        if len(job_paths) - len(finished_jobs) == 1:
            ...
            for job in hanging_job:
                ...
                self.logger.warning(
                    f"Created FINISHED file for {'hanging' if resolve_hangs else 'leaking memory'} job: {job}"
                )
        ...
        for info_file_path in info_file_paths:
            ...
            if pid:
                ...
                if memory_usage > (psutil.virtual_memory().total * 0.25):
                    ...
                    self.logger.warning(
                        f"Memory leak detected and resolved for job {pid}: {job_path}"
                    )
                    try:
                        time.sleep(20)
                        ...
                        self.logger.error(
                            f"Error terminating process with PID {pid}: {e}"
                        )
    ...
    def start_autobuild_tracking(self, main_pid, working_dir):
        ...
        self.process_tracking_file = os.path.join(
            working_dir, f"autobuild_tracking_{main_pid}.json"
        )
        tracking_data = {
            "main_pid": main_pid,
    ...
    def update_subjob_pids(self):
        ...
        if not self.process_tracking_file or not os.path.exists(
            self.process_tracking_file
        ):
            return
