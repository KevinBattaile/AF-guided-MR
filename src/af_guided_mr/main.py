...
from .molecular_replacement import MolecularReplacement
...
af_cluster_script_path = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "resources", "AF_cluster.py"
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Molecular replacement for multi-chain protein complexes using protein sequences and X-ray diffraction data."
    )
    parser.add_argument(
        "--csv_path",
        type=str,
        required=True,
        help="Path to the input CSV file containing protein sequences for each chain.",
    )
    parser.add_argument(
        "--mtz_path",
        type=str,
        required=True,
        help="Path to the input MTZ file containing reduced X-ray diffraction data.",
    )
    parser.add_argument(
        "--uniprot_ids",
        type=str,
        required=False,
        help="Comma-separated UniProt IDs corresponding to the order of sequences in the CSV file.",
    )
    parser.add_argument(
        "--copy_numbers",
        type=str,
        required=False,
        help="Collon-separated copy numbers corresponding to the order of sequences in the CSV file.",
    )
    parser.add_argument(
        "--solvent_content",
        type=float,
        required=False,
        help="Solvent content of the crystal (default: 0.5).",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        required=False,
        help="Base directory for all output files. If not specified, uses the current directory.",
    )
    parser.add_argument(
        "--nproc",
        type=int,
    ...
    parser.add_argument(
        "--no_timeout",
        action="store_true",
        help="Disable the 30 minutes timeout for the phaser run.",
    )
    parser.add_argument(
        "--force_af_cluster",
        action="store_true",
        help="Force running AF_cluster.py even if the output directory already exists, or for those shorter than 50 residue sequences.",
    )
    parser.add_argument(
        "--skip_af_cluster",
        action="store_true",
        help="Skip running AF_cluster.py, while using the models from pre-existing AF_cluster runs.",
    )
    parser.add_argument(
        "--skip_autobuild", action="store_true", help="Skip the autobuild process."
    )
    parser.add_argument(
        "--reference_model",
        type=str,
        required=False,
        help="Developing use: Path to the reference model for optional map-reference correlation.",
    )
    parser.add_argument(
        "--reference_map",
        type=str,
        required=False,
        help="Developing use: Path to the reference map in mtz format for optional map-reference correlation.",
    )
...
def main():
    ...
    def process_and_update_ensemble_files(
        protein_info, sequence_id, ensemble_type, b_factor_cutoff
    ):
        ...
        ensemble_files = protein_info[sequence_id][inference_type][
            f"{ensemble_type}_ensembles"
        ]
        processed_ensemble_dir = protein_info[sequence_id]["mr_models"][
            f"mr_model_folder_{domain_recognition_method}_mode"
        ]
        ...
        for file_path in ensemble_files:
            processed_file_path = pdb_manager.process_pdb_file(
                file_path,
                b_factor_cutoff,
                os.path.join(processed_ensemble_dir, os.path.basename(file_path)),
            )
            processed_ensemble_paths.append(processed_file_path)

        protein_info[sequence_id]["mr_models"][
            f"mr_model_path_{domain_recognition_method}_mode"
        ] = processed_ensemble_paths
    ...
    for idx, (sequence_id, sequence) in enumerate(sequences):
        ...
        protein_info[sequence_id] = {
            ...
            "af": {
                ...
                "alphafold_pae_domains_output_dir": os.path.join(
                    output_root, f"alphafold_pae_domains_output_{sequence_id}"
                ),
                "alphafold_interpro_domains_output_dir": os.path.join(
                    output_root, f"alphafold_interpro_domains_output_{sequence_id}"
                ),
            },
            "cf": {
                ...
                "colabfold_pae_domains_output_dir": os.path.join(
                    output_root, f"colabfold_pae_domains_output_{sequence_id}"
                ),
                "colabfold_interpro_ensembles": None,
                "colabfold_interpro_domains_output_dir": os.path.join(
                    output_root, f"colabfold_interpro_domains_output_{sequence_id}"
                ),
            },
            "best_model_path": None,
            "mr_models": {
                "mr_model_folder_default_mode": os.path.join(
                    output_root, "mr_models", sequence_id, "default_mode"
                ),
                "mr_model_path_default_mode": {"best": None, "second_best": None},
                "mr_model_folder_interpro_mode": os.path.join(
                    output_root, "mr_models", sequence_id, "interpro_mode"
                ),
                "mr_model_path_interpro_mode": None,
                "mr_model_folder_pae_mode": os.path.join(
                    output_root, "mr_models", sequence_id, "pae_mode"
                ),
    ...
    sequence_ids_over_50 = {
        seq_id
        for seq_id, details in protein_info.items()
        if len(details.get("sequence", "")) > 50
    }

    if len(sequences) != len(uniprot_ids):
        logging.warning(
            "Number of sequences and UniProt IDs do not match, which may result in Interpro mode not right."
        )
        # sys.exit(1)
    ...
    for sequence_id, protein_data in protein_info.items():
        ...
        if uniprot_id:
            logging.info(
                f"Processing sequence {sequence_id} with UniProt ID {uniprot_id}..."
            )
        else:
            logging.error(
                f"Could not process sequence {sequence_id} because no UniProt ID was supplied."
            )
        output_dir = os.path.join(output_root, sequence_id)
        os.makedirs(output_dir, exist_ok=True)
        ...
        if not uniprot_id and len(protein_info[sequence_id]["sequence"]) > 100:
            ...
            uniprot_id = sequence_manager.get_uniprot_id_with_timeout(
                protein_info[sequence_id]["sequence"]
            )

            if uniprot_id:
                logging.info(
                    f"Deduced Uniprot ID for sequence {sequence_id}: {uniprot_id}"
                )
            else:
                logging.warning(
                    f"Could not fetch Uniprot ID for sequence {sequence_id}."
                )
        ...
        if uniprot_id:
            try:
                ...
                if (
                    interpro_domains
                ):  # and "entry_protein_locations" in interpro_domains:
                    protein_info[sequence_id]["interpro_domains"] = interpro_domains
                    uniprot_start, uniprot_end = (
                        sequence_manager.get_absolute_positions(
                            protein_info[sequence_id]["sequence"], uniprot_id
                        )
                    )
                    ...
                    logging.warning(
                        f"No interpro domains found or data fetching failed for UniProt ID {uniprot_id}. Skipping interpro mode for sequence {sequence_id}."
                    )
                    uniprot_start = None
                ...
                logging.error(
                    f"Error processing sequence {sequence_id} with UniProt ID {uniprot_id}: {e}"
                )
        ...
        try:
            alphafold_model_path, alphafold_pae_json_path = (
                pdb_manager.download_alphafold_model(uniprot_id, output_dir)
            )
            if alphafold_model_path:
                model_sequence = pdb_manager.get_sequence_from_pdb(alphafold_model_path)
                start, end = sequence_manager.align_sequences(
                    protein_info[sequence_id]["sequence"], model_sequence
                )
                ...
                truncated_model_path = pdb_manager.truncate_model(
                    alphafold_model_path, start, end
                )
                alphafold_model_path = truncated_model_path
                protein_info[sequence_id]["af"][
                    "alphafold_model_path"
                ] = alphafold_model_path
                protein_info[sequence_id]["af"][
                    "alphafold_pae_json_path"
                ] = alphafold_pae_json_path
            else:
            ...
            logging.warning(
                f"Could not download AlphaFold model for sequence {sequence_id}: {e}"
            )
            alphafold_model_path = None
            alphafold_pae_json_path = None
        ...
        if not job_monitor.mark_colabfold_finished(output_dir, sequence_id):
            ...
            sequence_manager.write_custom_sequence_to_fasta(
                sequence_id, protein_info[sequence_id]["sequence"], fasta_filename
            )
            # run colabfold
            ...
            logging.info(
                f"ColabFold already run for sequence {sequence_id}. Skipping..."
            )
        colabfold_model_files = glob.glob(
            f"{output_dir}/{sequence_id}_relaxed_rank_001*seed_000.pdb"
        )
        colabfold_model_path = (
            colabfold_model_files[0] if colabfold_model_files[0] else None
        )
        colabfold_pae_json_path = (
            glob.glob(f"{output_dir}/{sequence_id}*predicted_aligned_error_v1.json")[0]
            if glob.glob(f"{output_dir}/{sequence_id}*predicted_aligned_error_v1.json")[
                0
            ]
            else None
        )
        ...
        colabfold_msa = (
            f"{output_dir}/{sequence_id}.a3m"
            if f"{output_dir}/{sequence_id}.a3m"
            else None
        )
        ...
        if colabfold_model_path:
            ...
            protein_info[sequence_id]["cf"][
                "colabfold_pae_json_path"
            ] = colabfold_pae_json_path
            # renumber colabfold model
            offset = (
                uniprot_start
                if uniprot_start is not None
                else (start if start is not None else 1)
            )
            best_model_path = pdb_manager.renumber_colabfold_model(
                colabfold_model_path, offset
            )
            ...
            print(
                f"AlphaFold model path: {protein_info[sequence_id]['af']['alphafold_model_path']}"
            )
            ...
            if protein_info[sequence_id]["af"]["alphafold_model_path"] is not None:
                alphafold_plddt = utilities.calculate_mean_plddt(
                    protein_info[sequence_id]["af"]["alphafold_model_path"]
                )
                ...
                if alphafold_plddt > colabfold_plddt:
                    ...
                    best_model_path = protein_info[sequence_id]["af"][
                        "alphafold_model_path"
                    ]
        ...
        if best_model_path:
            ...
            mr_model_folder_default_mode = protein_data.get("mr_models", {}).get(
                "mr_model_folder_default_mode"
            )
            if mr_model_folder_default_mode:
                ...
                logging.error(
                    f"Path for mr_model_folder_default_mode is None for sequence {sequence_id}"
                )
            base_name = os.path.basename(best_model_path)
            output_pdb_path = os.path.join(
                mr_model_folder_default_mode, f"{base_name}_plddt{b_factor_cutoff}.pdb"
            )

            protein_info[sequence_id]["mr_models"]["mr_model_path_default_mode"][
                "best"
            ] = pdb_manager.process_pdb_file(
                best_model_path, b_factor_cutoff, output_pdb_path
            )
            if second_best_model_path is not None:
                base_name = os.path.basename(second_best_model_path)
                output_pdb_path = os.path.join(
                    mr_model_folder_default_mode,
                    f"{base_name}_plddt{b_factor_cutoff}.pdb",
                )
                protein_info[sequence_id]["mr_models"]["mr_model_path_default_mode"][
                    "second_best"
                ] = pdb_manager.process_pdb_file(
                    second_best_model_path, b_factor_cutoff, output_pdb_path
                )
            if len(protein_info[sequence_id]["sequence"]) > 100:
                if colabfold_marker == True:
                    protein_info[sequence_id]["cf"][
                        "colabfold_adjusted_interpro_domains"
                    ] = protein_info[sequence_id]["interpro_domains"]
                    protein_info[sequence_id]["cf"][
                        "colabfold_adjusted_pae_domains"
                    ] = pdb_manager.get_domain_definitions_from_pae(
                        protein_info[sequence_id]["cf"]["colabfold_pae_json_path"],
                        offset,
                    )
                    # truncate colabfold model with interpro domains
                    colabfold_interpro_domains_output_dir = protein_data.get(
                        "cf", {}
                    ).get("colabfold_interpro_domains_output_dir")
                    os.makedirs(colabfold_interpro_domains_output_dir, exist_ok=True)
                    protein_info[sequence_id]["cf"]["colabfold_interpro_ensembles"] = (
                        pdb_manager.prepare_domain_ensembles(
                            best_model_path,
                            protein_info[sequence_id]["cf"][
                                "colabfold_adjusted_interpro_domains"
                            ],
                            colabfold_interpro_domains_output_dir,
                            len(protein_info[sequence_id]["sequence"]),
                        )
                    )
                    process_and_update_ensemble_files(
                        protein_info, sequence_id, "colabfold_interpro", b_factor_cutoff
                    )

                    # truncate colabfold model with pae domains
                    os.makedirs(
                        protein_info[sequence_id]["cf"][
                            "colabfold_pae_domains_output_dir"
                        ],
                        exist_ok=True,
                    )
                    protein_info[sequence_id]["cf"]["colabfold_pae_ensembles"] = (
                        pdb_manager.prepare_domain_ensembles(
                            best_model_path,
                            protein_info[sequence_id]["cf"][
                                "colabfold_adjusted_pae_domains"
                    ...
                    process_and_update_ensemble_files(
                        protein_info, sequence_id, "colabfold_pae", b_factor_cutoff
                    )

                else:
                    protein_info[sequence_id]["af"]["alphafold_pae_domains"] = (
                        pdb_manager.get_domain_definitions_from_pae(
                    ...
                    os.makedirs(
                        protein_info[sequence_id]["af"][
                            "alphafold_interpro_domains_output_dir"
                        ],
                        exist_ok=True,
                    )
                    protein_info[sequence_id]["af"]["alphafold_interpro_ensembles"] = (
                        pdb_manager.prepare_domain_ensembles(
                            best_model_path,
                            protein_info[sequence_id]["interpro_domains"],
                            protein_info[sequence_id]["af"][
                    ...
                    process_and_update_ensemble_files(
                        protein_info, sequence_id, "alphafold_interpro", b_factor_cutoff
                    )

                    # truncate alphafold model with pae domains
                    os.makedirs(
                        protein_info[sequence_id]["af"][
                            "alphafold_pae_domains_output_dir"
                        ],
                        exist_ok=True,
                    )
                    protein_info[sequence_id]["af"]["alphafold_pae_ensembles"] = (
                        pdb_manager.prepare_domain_ensembles(
                            best_model_path,
                            protein_info[sequence_id]["af"]["alphafold_pae_domains"],
                            protein_info[sequence_id]["af"][
                    ...
                    process_and_update_ensemble_files(
                        protein_info, sequence_id, "alphafold_pae", b_factor_cutoff
                    )

            else:
                logging.warning(
                    f"Sequence {sequence_id} is too short to run interpro/pae mode. Skipping..."
                )
        else:
            logging.warning(
                f"No suitable model found for sequence {sequence_id}. Skipping this sequence."
            )
    ...
    if args.copy_numbers:
        ...
        if len(copy_numbers_list) != len(sequence_ids):
            logging.error(
                "Number of specified copy numbers does not match the number of sequences in the CSV file."
            )
            sys.exit(1)
        specified_copy_numbers = {
            sequence_id: int(copy_num)
            for sequence_id, copy_num in zip(sequence_ids, copy_numbers_list)
        }
    else:
    ...
    closest_combination, all_combinations, mean_matthews_coeff = (
        molecular_replacement.multi_analyze_asu_and_solvent_content(
    ...
    if closest_combination:
        favorable_combination_str, prob, solvent_content, matthews_coeff = (
            closest_combination
        )
        if args.solvent_content:
            solvent_content = args.solvent_content
        updated_solvent_content = solvent_content
        copy_numbers_favorable = {
            protein: int(copy_num)
            for protein, copy_num in (
                protein_copy_combo.split(" x ")
                for protein_copy_combo in favorable_combination_str.split(", ")
            )
        }

        logging.info(
            f"Favorable combination of copy numbers: {favorable_combination_str}"
        )
        ...
        logging.error(
            "Could not find a favorable combination of copy numbers. Please supply the copy numbers using the --copy_numbers argument."
        )
        sys.exit(1)
    ...
    for sequence_id in protein_info:
        protein_info[sequence_id]["copy_number"] = copy_numbers_favorable.get(
            sequence_id, 1
        )

    mr_model_paths = {
        sequence_id: protein_info[sequence_id]["mr_models"][
            "mr_model_path_default_mode"
        ]["best"]
        for sequence_id in protein_info
    }
    copy_numbers = {
        sequence_id: protein_info[sequence_id]["copy_number"]
        for sequence_id in protein_info
    }
    ...
    phaser_info = {
        "default_mode": {
            ...
            "params_file": {
                "01": os.path.join(
                    output_root, "default_phaser_output01", "default_phaser.params"
                ),
                "02": os.path.join(
                    output_root, "default_phaser_output02", "default_phaser.params"
                ),
            },
            "phaser_log": {
                "01": os.path.join(
                    output_root, "default_phaser_output01", "PHASER.log"
                ),
                "02": os.path.join(
                    output_root, "default_phaser_output02", "PHASER.log"
                ),
            },
            "tfz_score": 0,
        },
        "interpro_mode": {
            ...
            "params_file": os.path.join(
                output_root, "interpro_phaser_output", "interpro_phaser.params"
            ),
            "tfz_score": 0,
            "interpro_switch": "off",
        },
        "pae_mode": {
            ...
            "params_file": os.path.join(
                output_root, "pae_phaser_output", "pae_phaser.params"
            ),
            "tfz_score": 0,
            "pae_switch": "off",
        },
    ...
    for sequence_id, copy_number in copy_numbers.items():
        total_sequence_length += (
            len(protein_info[sequence_id]["sequence"]) * copy_number
        )
        found_sequence_length += (
            len(protein_info[sequence_id]["sequence"]) * total_found_copies[sequence_id]
        )
    logging.info(
        f"Phaser default mode 1st attempt-Total sequence length: {total_sequence_length}, found sequence length: {found_sequence_length}"
    )
    if os.path.exists(phaser_info["default_mode"]["output_dir"]["01"]):
        logging.info(
            "default phaser output directory already exists. default phaser mode won't be run."
        )
        logging.warning(
            f"if you want to run default phaser mode, please delete the directory {phaser_info['default_mode']['output_dir']['01']} and rerun the script."
        )
    else:
        os.makedirs(phaser_info["default_mode"]["output_dir"]["01"], exist_ok=True)
        logging.info(
            f"running phaser molecular replacement with default mode, see parameters used in {phaser_info['default_mode']['params_file']['01']}..."
        )
        ...
        molecular_replacement.generate_phaser_params_multimer(
            phaser_info["default_mode"]["params_file"]["01"],
            args.mtz_path,
            solvent_content,
            space_group,
            mr_model_paths,
        ...
        molecular_replacement.run_phaser_molecular_replacement_async(
            phaser_info["default_mode"]["params_file"]["01"],
            phaser_info["default_mode"]["output_dir"]["01"],
            ignore_timeout=args.no_timeout,
        )

    phaser_info["default_mode"]["success"]["01"] = (
        molecular_replacement.handle_phaser_output(
            phaser_info["default_mode"]["output_dir"]["01"]
        )
    )
    ...
    if (
        not phaser_info["default_mode"]["success"]["01"]
        and protein_info[sequence_id]["mr_models"]["mr_model_path_default_mode"][
            "second_best"
        ]
        is not None
    ):
        logging.warning(
            "default phaser molecular replacement 1st attempt with pLDDT best model(s) not yielding useful result."
        )
        logging.info(
            f"Testing default phaser molecular replacement 1st attempt with pLDDT second best model(s): {protein_info[sequence_id]['mr_models']['mr_model_path_default_mode']['second_best']}"
        )
        mr_model_paths = {
            sequence_id: (
                protein_info[sequence_id]["mr_models"]["mr_model_path_default_mode"][
                ...
                if protein_info[sequence_id]["mr_models"]["mr_model_path_default_mode"][
                    "second_best"
                ]
                is not None
                else protein_info[sequence_id]["mr_models"][
                    "mr_model_path_default_mode"
                ]["best"]
            )
            for sequence_id in protein_info
        ...
        save_folder = os.path.join(
            phaser_info["default_mode"]["output_dir"]["01"], "save"
        )
        save_00_folder = os.path.join(
            phaser_info["default_mode"]["output_dir"]["01"], "save_00"
        )
        if os.path.exists(save_folder) and os.listdir(save_folder):
            ...
            for item in os.listdir(phaser_info["default_mode"]["output_dir"]["01"]):
                item_path = os.path.join(
                    phaser_info["default_mode"]["output_dir"]["01"], item
                )
                if item != "save":
    ...
    if phaser_info["default_mode"]["success"]["01"]:
        phaser_info["default_mode"]["tfz_score"], LLG = (
            molecular_replacement.get_final_tfz(
                phaser_info["default_mode"]["output_dir"]["01"]
            )
        )
        logging.success(
            f"TFZ score for default phaser run: {phaser_info['default_mode']['tfz_score']}, LLG: {LLG}"
        )
        logging.success(
            "default phaser molecular replacement 1st attempt successful, because at least some components were found.\nAnalyzing phaser output looking for potential adjustment..."
        )

        # Parse the Phaser log file to find out which chains to keep
        phaser_log_path = phaser_info["default_mode"]["phaser_log"][
            "01"
        ]  # Ensure this path is set correctly
        phaser_output_pdb = os.path.join(
            phaser_info["default_mode"]["output_dir"]["01"], "PHASER.1.pdb"
        )  # Path to Phaser's output PDB
        partial_pdb_path = os.path.join(
            phaser_info["default_mode"]["output_dir"]["01"], "default_mode_partial.pdb"
        )
        ...
        pdb_manager.process_pdb_file_for_phaser(
            phaser_output_pdb, keep_chains, partial_pdb_path
        )
        ...
        if kept_chain_count != total_expected_chains:
            ...
            updated_copy_numbers, updated_solvent_content, found_copies = (
                molecular_replacement.deduce_missing_copies(
                    phaser_log_path, phaser_info, all_combinations, mean_matthews_coeff
                )
            )
            if args.copy_numbers:
            ...
            all_sequences_found = all(
                seq_id in found_copies and found_copies[seq_id] > 0
                for seq_id in sequence_ids_over_50
            )  # a boolean indicating whether each sequence ID in sequence_ids_over_50 has at least one chain found in the first Phaser attempt.
            ...
            if all_sequences_found and updated_copy_numbers is not None:
                logging.info(
                    f"All components successfully located at least once for each, according to all_sequences_found: {all_sequences_found}. Found copies: {found_copies}."
                )
                ...
                """
                logging.info(
                    f"Updated copy numbers for 2nd default attempt: {updated_copy_numbers}"
                )
                ...
                if (
                    ...
                    or total_found_copies != updated_copy_numbers
                ):
                    logging.warning(
                        f"1st default phaser molecular replacement was partially successful. Preparing for a 2nd default phaser molecular replacement with updated copy numbers. {updated_copy_numbers}"
                    )

                    # Calculate missing copies for the second Phaser run
                    missing_copies = {
                        protein_id: updated_copy_numbers[protein_id]
                        - found_copies.get(protein_id, 0)
                        for protein_id in updated_copy_numbers
                    }
                    logging.info(
                        f"Missing copies for 2nd default attempt: {missing_copies}"
                    )
                    ...
                    if all(value == 0 for value in total_missing_copies.values()):
                        logging.info(
                            "default phaser molecular replacement are considered successful, although there may be some components missing, further visualization of the output pdb and map is needed."
                        )
                        phaser_info["interpro_mode"]["interpro_switch"] = "off"

                    else:
                        if os.path.exists(
                            phaser_info["default_mode"]["output_dir"]["02"]
                        ):
                            logging.info(
                                "default phaser output directory already exists. default phaser mode won't be run."
                            )
                            logging.warning(
                                f"if you want to run default phaser mode, please delete the directory {phaser_info['default_mode']['output_dir']['02']} and rerun the script."
                            )
                        else:
                            ...
                            molecular_replacement.generate_phaser_params_for_second_run(
                                phaser_info["default_mode"]["params_file"]["02"],
                                args.mtz_path,
                                updated_solvent_content,
                                space_group,
                                partial_pdb_path,
                                mr_model_paths,
                                missing_copies,
                        ...
                        phaser_info["default_mode"]["success"]["02"] = (
                            molecular_replacement.handle_phaser_output(
                                phaser_info["default_mode"]["output_dir"]["02"]
                            )
                        )
                        default_2nd_output_pdb = os.path.join(
                            phaser_info["default_mode"]["output_dir"]["02"],
                            "PHASER.1.pdb",
                        )  # Path to Phaser's output PDB
                        if phaser_info["default_mode"]["success"]["02"]:
                            ...
                            logging.success(
                                f"TFZ score for default phaser run round 2: {phaser_info['default_mode']['tfz_score']}, LLG: {LLG}"
                            )
                            logging.success(
                                "default phaser molecular replacement 2nd attempt successful, because all the rest components were found.\nAnalyzing phaser output looking for potential adjustment..."
                            )

                            # Check if the newly found copies match the missing copies from the first attempt
                            _, _, new_found_copies = (
                                molecular_replacement.deduce_missing_copies(
                                    phaser_info["default_mode"]["phaser_log"]["02"],
                                    phaser_info,
                                    all_combinations,
                                    mean_matthews_coeff,
                                )
                            ...
                            logging.info(
                                f"Newly found copies during 2nd phaser attemp: {new_found_copies}"
                            )
                            total_found_copies = {
                                protein_id: total_found_copies.get(protein_id, 0)
                                + new_found_copies.get(protein_id, 0)
                                for protein_id in total_found_copies
                            }
                            logging.info(
                                f"Total found copies after 2nd default mode: {total_found_copies}"
                            )
                            if new_found_copies == missing_copies:
                                logging.success(
                                    f"All components successfully located: {total_found_copies}. No further searches needed."
                                )
                                phaser_info["default_mode"]["success"]["02"] = True
                            else:
                                logging.warning(
                                    "Not all components located. Preparing for Interpro mode."
                                )
                                # Update copy numbers for Interpro mode
                                interpro_copy_numbers = {
                                    protein_id: missing_copies.get(protein_id, 0)
                                    - new_found_copies.get(protein_id, 0)
                                    for protein_id in missing_copies
                                }
                                print(
                                    f"Interpro copy numbers not equals missing_copies: {interpro_copy_numbers}"
                                )
                                total_missing_copies = interpro_copy_numbers
                                phaser_info["interpro_mode"][
                                    "interpro_switch"
                                ] = "on"  # switch to interpro mode
                            # Generate new partial pdb for Interpro mode if necessary
                            keep_chains = pdb_manager.parse_phaser_log(
                                phaser_info["default_mode"]["phaser_log"]["02"]
                            )
                            default_2nd_partial_pdb_path = os.path.join(
                                phaser_info["default_mode"]["output_dir"]["02"],
                                "default_mode_2nd_partial.pdb",
                            )
                            pdb_manager.process_pdb_file_for_phaser(
                                default_2nd_output_pdb,
                                keep_chains,
                                default_2nd_partial_pdb_path,
                                partial_pdb_path,
                            )
                            ...
                            logging.warning(
                                "2nd default phaser molecular replacement failed. Will carry on with interpro/pae mode."
                            )
                            ...
                            print(
                                f"Interpro copy numbers equals missing_copies : {interpro_copy_numbers}"
                            )
                            ...
                            phaser_results.append(
                                {
                                    ...
                                    "phaser_output_dir": phaser_info["default_mode"][
                                        "output_dir"
                                    ]["02"],
                                    "phaser_output_pdb": os.path.join(
                                        phaser_info["default_mode"]["output_dir"]["02"],
                                    ...
                                    "phaser_output_map": os.path.join(
                                        phaser_info["default_mode"]["output_dir"]["02"],
                                        "PHASER.1.mtz",
                                    ),
                                }
                ...
                total_missing_copies = {
                    protein_id: total_copies_to_search.get(protein_id, 0)
                    - total_found_copies.get(protein_id, 0)
                    for protein_id in total_copies_to_search
                }
                logging.warning(
                    f"some components were found, but not all: all_sequences_found {all_sequences_found}. Preparing for Interpro mode."
                )
                phaser_info["interpro_mode"]["interpro_switch"] = "on"
            ...
            logging.success(
                "default phaser molecular replacement successful, because all components were found."
            )
            phaser_info["default_mode"]["success"]["01"] = True
            phaser_info["interpro_mode"]["interpro_switch"] = "off"

        if (
            partial_pdb_path is not None
            and pdb_manager.get_sequence_length_from_pdb(partial_pdb_path) > 0
        ):
            (
                r_work_default_mode,
                r_free_default_mode,
                refinement_folder_default_mode,
                _,
            ) = utilities.rfactors_from_phenix_refine(
                partial_pdb_path, args.mtz_path, refine_output_root, nproc=args.nproc
            )
            ...
            if r_free_default_mode < r_free_threshold:
                logging.success(
                    f"Default mode solution accepted based on R: {r_work_default_mode:.2f}/{r_free_default_mode:.2f}. refinement output: {refinement_folder_default_mode}"
                )
                # Check if each sequence has at least one chain found in the Phaser attempt default mode
                all_sequences_found = all(
                    seq_id in total_found_copies and total_found_copies[seq_id] > 0
                    for seq_id in sequence_ids_over_50
                )
                if all_sequences_found:
                    phaser_info["interpro_mode"]["interpro_switch"] = "off"
                    logging.success(
                        f"All components successfully located at least once for each: {total_found_copies}. The r_free value is {r_free_default_mode:.2f}. will not run interpro mode."
                    )
            else:
                logging.warning(
                    f"Default mode solution rejected based on R: {r_work_default_mode:.2f}/{r_free_default_mode:.2f}. refinement output: {refinement_folder_default_mode}"
                )
    ...
    if os.path.exists(
        os.path.join(phaser_info["default_mode"]["output_dir"]["01"], "PHASER.1.pdb")
    ):
        default_mode_tfz_score_01, _ = molecular_replacement.get_final_tfz(
            phaser_info["default_mode"]["output_dir"]["01"]
        )
        phaser_results.append(
            {
                ...
                "phaser_output_map": os.path.join(
                    phaser_info["default_mode"]["output_dir"]["01"], "PHASER.1.mtz"
                ),
            }
        )
    ...
    if partial_pdb_path is not None and os.path.exists(partial_pdb_path):
        default_phaser_output_pdb_residue_count = (
    ...
    existing_residue_count = (
        default_phaser_output_pdb_residue_count
        if default_phaser_output_pdb_residue_count
        else 0
    )
    """
    ...
    if phaser_info["interpro_mode"]["interpro_switch"] == "on":
        ...
        print(
            f"Interpro copy numbers which is the same as total_missing_copies: {interpro_copy_numbers}"
        )
        logging.info(
            f"Running phaser molecular replacement with interpro mode, see parameters used in {interpro_params_file}..."
        )
        logging.info(
            f"for interpro mode, copy numbers to search for are {interpro_copy_numbers}"
        )

        # Determine the copy numbers and MR models for Interpro mode
        interpro_mr_models = {
            sequence_id: protein_info[sequence_id]["mr_models"][
                "mr_model_path_interpro_mode"
            ]
            for sequence_id in total_missing_copies
        }
        ...
        if not interpro_mr_models:
            ...
            if os.path.exists(interpro_mode_dir):
                logging.info(
                    "interpro phaser output directory already exists. interpro phaser mode won't be run."
                )
                logging.warning(
                    f"if you want to run interpro phaser mode, please delete the directory {interpro_mode_dir} and rerun the script."
                )
            else:
                ...
                if not phaser_info["default_mode"]["success"]["01"]:
                    ...
                    interpro_mr_models = {
                        protein_id: protein_info[protein_id]["mr_models"][
                            "mr_model_path_interpro_mode"
                        ]
            ...
            phaser_info["interpro_mode"]["success"] = (
                molecular_replacement.handle_phaser_output(interpro_mode_dir)
            )
            if phaser_info["interpro_mode"]["success"]:
                ...
                interpro_phaser_output_pdb = os.path.join(
                    interpro_mode_dir, "PHASER.1.pdb"
                )  # Path to Phaser's output PDB
                interpro_partial_pdb_path = os.path.join(
                    interpro_mode_dir, "interpro_partial.pdb"
                )
                pdb_manager.process_pdb_file_for_phaser(
                    interpro_phaser_output_pdb,
                    keep_chains,
                    interpro_partial_pdb_path,
                    partial_pdb_path,
                )

                interpro_phaser_output_pdb_residue_count = (
                    pdb_manager.get_sequence_length_from_pdb(interpro_phaser_output_pdb)
                    if interpro_phaser_output_pdb
                    else 0
                )
                interpro_partial_pdb_path_residue_count = (
                    pdb_manager.get_sequence_length_from_pdb(interpro_partial_pdb_path)
                    if interpro_partial_pdb_path
                    else 0
                )
                logging.info(
                    f"Residue count of interpro_phaser_output_pdb: {interpro_phaser_output_pdb_residue_count}, residue count of interpro_partial_pdb_path: {interpro_partial_pdb_path_residue_count}."
                )
                logging.info(
                    f"Residue count of default_phaser_output_pdb: {default_phaser_output_pdb_residue_count}."
                )

                if (
                    interpro_partial_pdb_path_residue_count
                    > default_phaser_output_pdb_residue_count
                    and interpro_partial_pdb_path_residue_count != 0
                ):
                    phaser_info["interpro_mode"]["tfz_score"], LLG = (
                        molecular_replacement.get_final_tfz(interpro_mode_dir)
                    )
                    logging.info(
                        f"TFZ score for interpro phaser run: {phaser_info['interpro_mode']['tfz_score']}, LLG: {LLG}"
                    )
                    (
                        r_work_interpro_mode,
                        r_free_interpro_mode,
                        refinement_folder_interpro_mode,
                        _,
                    ) = utilities.rfactors_from_phenix_refine(
                        interpro_partial_pdb_path,
                        args.mtz_path,
                    ...
                    logging.info(
                        f"interpro mode refinement output: {refinement_folder_interpro_mode}"
                    )
                    refinement_results.append(
                        RefinementResult(
                    ...
                    if r_free_interpro_mode < r_free_threshold:
                        if (
                            r_free_default_mode < r_free_threshold
                            and r_free_interpro_mode > r_free_default_mode
                        ):
                            logging.warning(
                                f"Interpro mode solution with R: {r_work_interpro_mode:.2f}/{r_free_interpro_mode:.2f} is worse than default mode solution with R: {r_work_default_mode:.2f}/{r_free_default_mode:.2f}"
                            )
                            # no need to update the total_missing_copies and total_found_copies and partial_pdb_path, log them for debugging
                            logging.info(
                                f"total_missing_copies after interpro mode [worse]: {total_missing_copies}"
                            )
                            logging.info(
                                f"total_found_copies after interpro mode [worse]: {total_found_copies}"
                            )
                            logging.info(
                                f"partial_pdb_path after interpro mode [worse]: {partial_pdb_path}"
                            )
                            ...
                            logging.success(
                                f"Interpro mode solution accepted based on R: {r_work_interpro_mode:.2f}/{r_free_interpro_mode:.2f}"
                            )
                            logging.info(
                                f"After interpro mode, all combinations of copy numbers are {all_combinations}"
                            )
                            ...
                            logging.info(
                                f"Newly found copies during interpro phaser attempt: {new_found_copies}"
                            )
                            total_found_copies = {
                                protein_id: total_found_copies.get(protein_id, 0)
                                + new_found_copies.get(protein_id, 0)
                            ...
                            logging.info(
                                f"Total found copies after interpro mode: {total_found_copies}"
                            )
                            total_missing_copies = {
                                protein_id: total_missing_copies.get(protein_id, 0)
                                - new_found_copies.get(protein_id, 0)
                                for protein_id in total_missing_copies
                            }
                            if new_found_copies == interpro_copy_numbers:
                                logging.success(
                                    "All components successfully located. No further searches needed."
                                )
                                phaser_info["interpro_mode"]["success"] = True
                            else:
                                logging.warning(
                                    "Not all components located. Preparing for pae mode."
                                )
                                phaser_info["pae_mode"]["pae_switch"] = "on"
                        ...
                        logging.warning(
                            f"Interpro mode solution rejected based on R: {r_work_interpro_mode:.2f}/{r_free_interpro_mode:.2f}"
                        )
                        phaser_info["interpro_mode"]["success"] = False
                        phaser_info["pae_mode"]["pae_switch"] = "on"
                        # there should be not need to update the total_missing_copies and total_found_copies, log them for debugging
                        logging.info(
                            f"total_missing_copies after interpro mode [rejection]: {total_missing_copies}"
                        )
                        logging.info(
                            f"total_found_copies after interpro mode [rejection]: {total_found_copies}"
                        )
                        logging.info(
                            f"partial_pdb_path after interpro mode [rejection]: {partial_pdb_path}"
                        )
                elif interpro_partial_pdb_path_residue_count == existing_residue_count:
                    logging.info(
                        "Interpro mode did not find new components. Proceeding with pae mode."
                    )
                    phaser_info["pae_mode"]["pae_switch"] = "on"
            else:
                ...
                phaser_info["interpro_mode"]["tfz_score"], _ = (
                    molecular_replacement.get_final_tfz(interpro_mode_dir)
                )
                ...
                phaser_results.append(
                    {
                        ...
                        "phaser_output_map": os.path.join(
                            interpro_mode_dir, "PHASER.1.mtz"
                        ),
                    }
                )
    ...
    if phaser_info["pae_mode"]["pae_switch"] == "on":
        ...
        print(
            f"pae copy numbers which is the same as total_missing_copies: {pae_copy_numbers}"
        )

        # Skip pae mode if no models are found
        pae_mr_models = {
            sequence_id: protein_info[sequence_id]["mr_models"][
                "mr_model_path_pae_mode"
            ]
            for sequence_id in total_missing_copies
        }
        if not pae_mr_models:
            ...
            if os.path.exists(pae_mode_dir):
                logging.info(
                    "pae phaser output directory already exists. pae phaser mode won't be run."
                )
                logging.warning(
                    f"if you want to run pae phaser mode, please delete the directory {pae_mode_dir} and rerun the script."
                )
            else:
                os.makedirs(pae_mode_dir, exist_ok=True)
                logging.info(
                    f"Running phaser molecular replacement with pae mode, see parameters used in {pae_params_file}..."
                )
                logging.info(
                    f"for pae mode, copy numbers to search for are {pae_copy_numbers}"
                )
                ...
                molecular_replacement.run_phaser_molecular_replacement_async(
                    pae_params_file, pae_mode_dir, ignore_timeout=args.no_timeout
                )

            # Handle the output of the pae mode
            phaser_info["pae_mode"]["success"] = (
                molecular_replacement.handle_phaser_output(pae_mode_dir)
            )
            if phaser_info["pae_mode"]["success"]:
                ...
                pae_phaser_output_pdb = os.path.join(
                    pae_mode_dir, "PHASER.1.pdb"
                )  # Path to Phaser's output PDB
                pae_partial_pdb_path = os.path.join(pae_mode_dir, "pae_partial.pdb")
                pdb_manager.process_pdb_file_for_phaser(
                    pae_phaser_output_pdb,
                    keep_chains,
                    pae_partial_pdb_path,
                    partial_pdb_path,
                )
                ...
                if (
                    pae_partial_pdb_path_residue_count
                    != current_partial_pdb_path_residue_count
                    and pae_partial_pdb_path_residue_count != 0
                ):
                    # Retrieve TFZ score and LLG for pae mode
                    phaser_info["pae_mode"]["tfz_score"], LLG = (
                        molecular_replacement.get_final_tfz(pae_mode_dir)
                    )
                    logging.info(
                        f"TFZ score for pae phaser run: {phaser_info['pae_mode']['tfz_score']}, LLG: {LLG}"
                    )

                    # Perform refinement using Phenix
                    r_work_pae_mode, r_free_pae_mode, refinement_folder_pae_mode, _ = (
                        utilities.rfactors_from_phenix_refine(
                            pae_partial_pdb_path,
                            args.mtz_path,
                            refine_output_root,
                    ...
                    logging.info(
                        f"pae mode refinement output: {refinement_folder_pae_mode}"
                    )
                    refinement_results.append(
                        RefinementResult(
                    ...
                    if r_free_pae_mode < r_free_threshold:
                        if (
                            r_free_interpro_mode < r_free_threshold
                            and r_free_pae_mode > existing_r_free
                        ):
                            logging.warning(
                            ...
                            logging.info(
                                f"total_missing_copies after pae mode [worse]: {total_missing_copies}"
                            )
                            logging.info(
                                f"total_found_copies after pae mode [worse]: {total_found_copies}"
                            )
                            logging.info(
                                f"partial_pdb_path after pae mode [worse]: {partial_pdb_path}"
                            )
                            ...
                            logging.success(
                                f"Pae mode solution accepted based on R: {r_work_pae_mode:.2f}/{r_free_pae_mode:.2f}"
                            )
                            logging.info(
                                f"After pae mode, all combinations of copy numbers are {all_combinations}"
                            )
                            ...
                            logging.info(
                                f"Newly found copies during pae phaser attempt: {new_found_copies}"
                            )
                            ...
                            logging.info(
                                f"Total found copies after pae mode: {total_found_copies}"
                            )
                            ...
                            if new_found_copies == pae_copy_numbers:
                                ...
                                logging.warning(
                                    "Not all components located. Preparing for AF_cluster mode."
                                )
                                phaser_info["AF_cluster_mode"][
                                    "af_cluster_switch"
                                ] = "on"
                        ...
                        logging.warning(
                            f"Pae mode solution rejected based on R: {r_work_pae_mode:.2f}/{r_free_pae_mode:.2f}"
                        )
                        phaser_info["pae_mode"]["success"] = False
                        phaser_info["AF_cluster_mode"]["af_cluster_switch"] = "on"
                        ...
                        # Log debugging information without updating copies
                        logging.info(
                            f"total_missing_copies after pae mode [rejection]: {total_missing_copies}"
                        )
                        logging.info(
                            f"total_found_copies after pae mode [rejection]: {total_found_copies}"
                        )
                        logging.info(
                            f"partial_pdb_path after pae mode [rejection]: {partial_pdb_path}"
                        )
                    ...
                    logging.info(
                        "Pae mode did not find new components. Proceeding with AF_cluster mode."
                    )
                    phaser_info["AF_cluster_mode"]["af_cluster_switch"] = "on"
            else:
                ...
                phaser_info["pae_mode"]["tfz_score"], _ = (
                    molecular_replacement.get_final_tfz(pae_mode_dir)
                )
    ...
    if updated_copy_numbers is not None:
        ...
        for protein_id in updated_copy_numbers:
            total_sequence_length += (
                len(protein_info[protein_id]["sequence"])
                * updated_copy_numbers[protein_id]
            )
        logging.info(
            f"Total sequence length after interpro/pae mode: {total_sequence_length}"
        )
    ...
    if phaser_info["AF_cluster_mode"]["af_cluster_switch"] == "on":
        if partial_pdb_path is not None:
            found_sequence_length = pdb_manager.get_sequence_length_from_pdb(
                partial_pdb_path
            )
            logging.info(
                f"Found sequence length from partial pdb: {found_sequence_length}"
            )
        ...
        if found_ratio >= 0.7:
            ...
            logging.info(
                f"Found sequence length is at least 70% of the total sequence length: {found_sequence_length}/{total_sequence_length} = {found_ratio:.2f}. AF_cluster mode will not be run to save time."
            )
            ...
            if current_best_result.r_free < r_free_threshold:
                logging.info(
                    f"Best refinement result: {current_best_result.r_free:.2f} in {current_best_result.refinement_folder}, skipping AF_cluster mode to save time."
                )
                phaser_info["AF_cluster_mode"]["af_cluster_switch"] = "off"
    ...
    if phaser_info["AF_cluster_mode"]["af_cluster_switch"] == "on":
        ...
        if os.path.exists(AF_cluster_mode_root) and not args.skip_af_cluster:
            logging.info(
                "AF_cluster mode output directory already exists. AF_cluster mode won't be run."
            )
            logging.warning(
                f"if you want to run AF_cluster mode, please delete the directory {AF_cluster_mode_root} and rerun the script."
            )
        else:
            # for the protein in total_missing_copies, prepare the AF_cluster_mode_dir
            logging.info(
                f"total_missing_copies right before AF_cluster mode: {total_missing_copies}"
            )
            logging.info(
                f"total_found_copies right before AF_cluster mode: {total_found_copies}"
            )
            ...
            proteins_for_AF_cluster = {
                protein_id
                for protein_id in total_missing_copies
                if total_missing_copies[protein_id] > 0
                and protein_id in sequence_ids_over_50
            }
            ...
            sequence_lengths = {
                protein_id: len(protein_info[protein_id]["sequence"])
                for protein_id in protein_info
            }
            sorted_proteins_for_AF_cluster = sorted(
                proteins_for_AF_cluster,
                key=lambda protein_id: sequence_lengths[protein_id],
                reverse=True,
            )
            ...
            for protein_id in sorted_proteins_for_AF_cluster:
                ...
                af_cluster_reference_pdb_path = protein_info[protein_id]["mr_models"][
                    "mr_model_path_default_mode"
                ]["best"]
                if not args.skip_af_cluster:
                    logging.info(
                        f"Calling run_af_cluster with arguments: {protein_info[protein_id]['cf']['colabfold_msa']}, {af_cluster_reference_pdb_path}"
                    )
                    af_cluster_process = subprocess.Popen(
                        [
                            sys.executable,
                            af_cluster_script_path,
                            protein_info[protein_id]["cf"]["colabfold_msa"],
                            af_cluster_reference_pdb_path,
                            str(40),
                        ],
                        cwd=AF_cluster_mode_dir,
                    )
                elif args.skip_af_cluster:
                    logging.info(
                        "Skipping running new AF_cluster; using pre-existing AF_cluster models."
                    )
                    af_cluster_process = None
                AF_cluster_phaser_output_dir = phaser_info["AF_cluster_mode"][
                    "output_dir"
                ]

                predictions_folder = os.path.join(AF_cluster_mode_dir, "predictions")
                selectives_folder = os.path.join(
                    AF_cluster_mode_dir, "AF_cluster_selectives"
                )
                af_cluster_finish_file = os.path.join(
                    selectives_folder, "ALL_AFCLUSTER_DONE"
                )
                rmsd_ranking_file = os.path.join(selectives_folder, "rmsd_ranking.csv")
                AF_cluster_treated_pdb_dir = os.path.join(
                    AF_cluster_mode_dir, "AF_cluster_plddt_treated_pdbs"
                )
                af_cluster_phaser_base_dir = os.path.join(
                    AF_cluster_mode_dir, "AF_cluster_phaser_runs"
                )
                ...
                while not AF_cluster_success:
                    ...
                    successful_refinement = (
                        refinement_manager.check_completed_refinements()
                    )
                    if successful_refinement:
                        ...
                        phaser_output_dir = (
                            successful_refinement.phaser_output_dir
                        )  # Get the correct Phaser directory

                        logging.success(
                            f"AF_cluster mode solution accepted based on R: {r_work:.2f}/{r_free:.2f}"
                        )
                        ...
                        _, _, new_found_copies = (
                            molecular_replacement.deduce_missing_copies(
                                os.path.join(mr_cluster_dir, "PHASER.log"),
                                phaser_info,
                                all_combinations,
                                mean_matthews_coeff,
                                top_switch=False,
                        ...
                        logging.info(
                            f"Newly found copies during AF_cluster phaser attempt: {new_found_copies}"
                        )
                        ...
                        logging.info(
                            f"Total found copies after AF_cluster mode: {total_found_copies}"
                        )
                        ...
                        logging.info(
                            f"Total missing copies after AF_cluster mode: {total_missing_copies}"
                        )
                        ...
                        found_ratio = found_sequence_length / total_sequence_length
                        logging.info(
                            f"Current found sequence length: {found_sequence_length}, total sequence length: {total_sequence_length}, ratio: {found_ratio:.2f}"
                        )

                        # Check if all copies have been found
                        if (
                            all(value <= 0 for value in total_missing_copies.values())
                            or found_ratio >= 0.8
                        ):
                            logging.success(
                                "Either all components successfully located, or found ratio is at least 80%. No further searches needed."
                            )
                            ...
                            if af_cluster_process is not None:
                                ...
                                halt_file_path = os.path.join(
                                    predictions_folder, "HALT"
                                )
                            ...
                            logging.warning(
                                "Terminated running phaser process and pending refinements after finding successful solution"
                            )
                        else:
                            logging.warning(
                                "Not all components located. Further searches may be needed."
                            )
                    ...
                    if refinements:
                        ...
                        if (
                            not successful_refinement and best_refinement.r_free < 0.5
                        ):  # Use reasonable cutoff
                    ...
                    if os.path.exists(rmsd_ranking_file):
                        next_pdb = pdb_manager.get_next_pdb_entry(
                            rmsd_ranking_file, used_pdbs
                        )
                        if next_pdb is None and os.path.exists(af_cluster_finish_file):
                            logging.warning(
                                "All PDB entries from rmsd_ranking.csv have been tested with no success."
                            )
                            break
                        while next_pdb is None and not os.path.exists(
                            af_cluster_finish_file
                        ):
                            time.sleep(30)
                            next_pdb = pdb_manager.get_next_pdb_entry(
                                rmsd_ranking_file, used_pdbs
                        ...
                        if next_pdb is not None:
                            ...
                            # cluster_pdb_paths = []
                            cluster_cf_pdb_path = os.path.join(
                                selectives_folder, next_pdb
                            )
                            print(f"next pdb to be processed: {cluster_cf_pdb_path}")
                            cluster_number = re.search(
                                "cluster_(\d+)_", cluster_cf_pdb_path
                            ).group(1)
                            cluster_cf_pae_path = os.path.join(
                                selectives_folder,
                                f"cluster_{cluster_number}_predicted_aligned_error_v1.json",
                            )
                            ...
                            best_model_path = pdb_manager.renumber_colabfold_model(
                                cluster_cf_pdb_path, offset
                            )
                            mean_plddt_score = utilities.calculate_mean_plddt(
                                best_model_path
                            )
                            b_factor_cutoff = 40 if mean_plddt_score < 55 else 60
                            logging.info(
                                f"mean_plddt_score for {protein_id} cluster {cluster_number} is {int(mean_plddt_score)}, thus b_factor_cutoff is {b_factor_cutoff}"
                            )
                            # Generate and run Phaser parameters for AF_cluster mode
                            mr_cluster_dir = os.path.join(
                                af_cluster_phaser_base_dir, f"mr_{cluster_number}"
                            )
                            ...
                            cluster_mr_model_path = pdb_manager.process_pdb_file(
                                best_model_path, b_factor_cutoff, output_pdb_path
                            )
                            cluster_mr_model = {protein_id: cluster_mr_model_path}
                            params_filename = os.path.join(
                                mr_cluster_dir,
                                f"phaser_params_cluster_{cluster_number}.txt",
                            )
                            if partial_pdb_path is None:
                                molecular_replacement.generate_phaser_params_multimer(
                                    params_filename,
                                    ...
                                    space_group,
                                    cluster_mr_model,
                                    {protein_id: total_missing_copies[protein_id]},
                                    phaser_info,
                                    nproc=args.nproc,
                                )
                            else:
                            ...
                            logging.info(
                                f"phaser params file for cluster {cluster_number} is generated and run."
                            )
                            phaser_process = molecular_replacement.run_phaser_molecular_replacement_async(
                                params_filename,
                                mr_cluster_dir,
                                ignore_timeout=args.no_timeout,
                            )

                            AF_cluster_mr_success = (
                                molecular_replacement.handle_phaser_output(
                                    mr_cluster_dir
                                )
                            )
                            ...
                            if AF_cluster_mr_success:
                                ...
                                AF_cluster_phaser_output_pdb = os.path.join(
                                    mr_cluster_dir, "PHASER.1.pdb"
                                )  # Path to Phaser's output PDB
                                AF_cluster_partial_pdb_path = os.path.join(
                                    mr_cluster_dir,
                                    f"AF_cluster_partial_{cluster_number}.pdb",
                                )
                                pdb_manager.process_pdb_file_for_phaser(
                                    AF_cluster_phaser_output_pdb,
                                    keep_chains,
                                    AF_cluster_partial_pdb_path,
                                    partial_pdb_path,
                                )

                                af_cluster_tfz_score, _ = (
                                    molecular_replacement.get_final_tfz(mr_cluster_dir)
                                )
                                ...
                                """
                                logging.info(
                                    f"AF_cluster phaser run {cluster_number} failed, will try PAE mode for {protein_id}."
                                )
                                phaser_info["AF_cluster_mode"]["tfz_score"], _ = (
                                    molecular_replacement.get_final_tfz(mr_cluster_dir)
                                )
                                if os.path.exists(
                                    os.path.join(mr_cluster_dir, "PHASER.1.pdb")
                                ):
                                    phaser_results.append(
                                        {
                                            "mode": f"AF_cluster_mode_cluster_{cluster_number}",
                                            "tfz_score": phaser_info["AF_cluster_mode"][
                                                "tfz_score"
                                            ],
                                            "phaser_output_dir": mr_cluster_dir,
                                            "phaser_output_pdb": os.path.join(
                                            ...
                                            "phaser_output_map": os.path.join(
                                                mr_cluster_dir, "PHASER.1.mtz"
                                            ),
                                        }
                                    )

                                mr_cluster_ensemble_dir = os.path.join(
                                    mr_cluster_dir, "mr_cluster_ensemble"
                                )
                                cf_adjusted_pae_domains = (
                                    pdb_manager.get_domain_definitions_from_pae(
                                        cluster_cf_pae_path,
                                        offset,
                                        primary_pae_cutoff=15,
                                    )
                                )
                                # logging.info(f"cf_adjusted_pae_domains for {cluster_number}: {cf_adjusted_pae_domains}")
                                cluster_ensemble_files = (
                                    pdb_manager.prepare_domain_ensembles(
                                        best_model_path,
                                        cf_adjusted_pae_domains,
                                        mr_cluster_ensemble_dir,
                                        len(protein_info[protein_id]["sequence"]),
                                    )
                                )
                                logging.info(
                                    f"cluster_ensemble_files: {cluster_ensemble_files}"
                                )
                                processed_ensemble_dir = os.path.join(
                                    mr_cluster_ensemble_dir,
                                    "processed_ensembles_for_pae_mr",
                                )
                                os.makedirs(processed_ensemble_dir, exist_ok=True)
                                processed_ensemble_paths = {protein_id: []}
                                for ensemble_file in cluster_ensemble_files:
                                    processed_ensemble_path = os.path.join(
                                        processed_ensemble_dir,
                                        os.path.basename(ensemble_file),
                                    )
                                    pdb_manager.process_pdb_file(
                                        ensemble_file,
                                        b_factor_cutoff,
                                    ...
                                    processed_ensemble_paths[protein_id].append(
                                        processed_ensemble_path
                                    )
                                params_filename = os.path.join(
                                    mr_cluster_ensemble_dir,
                                    f"phaser_params_cluster_{cluster_number}_pae_mode.txt",
                                )
                                if partial_pdb_path is None:
                                    molecular_replacement.generate_phaser_params_multimer(
                                        params_filename,
                                ...
                                logging.info(
                                    f"phaser params file for cluster {cluster_number} in pae mode is generated and run."
                                )
                                phaser_process = molecular_replacement.run_phaser_molecular_replacement_async(
                                    params_filename,
                                    mr_cluster_ensemble_dir,
                                ...
                                if AF_cluster_pae_success:
                                    ...
                                    keep_chains = pdb_manager.parse_phaser_log(
                                        os.path.join(
                                            mr_cluster_ensemble_dir, "PHASER.log"
                                        )
                                    )
                                    AF_cluster_phaser_output_pdb = os.path.join(
                                        mr_cluster_ensemble_dir, "PHASER.1.pdb"
                                    )
                                    AF_cluster_partial_pdb_path = os.path.join(
                                        mr_cluster_ensemble_dir,
                                        f"AF_cluster_partial_{cluster_number}.pae.pdb",
                                    )
                                    pdb_manager.process_pdb_file_for_phaser(
                                        AF_cluster_phaser_output_pdb,
                                        keep_chains,
                                    ...
                                    logging.warning(
                                        f"AF_cluster phaser run {cluster_number} in pae mode failed."
                                    )
                                    # phaser_info["AF_cluster_mode"]['tfz_score'] = 0
                                    logging.info(
                                        f"partial_pdb_path remains as: {partial_pdb_path}"
                                    )
                                    # No need to update total_missing_copies and total_found_copies
                                    phaser_info["AF_cluster_mode"]["tfz_score"], _ = (
                                        molecular_replacement.get_final_tfz(
                ...
                if all(count == 0 for count in total_missing_copies.values()):
                    logging.success(
                        f"All components successfully located. Total missing copies: {total_missing_copies}.No further searches needed. Total found copies: {total_found_copies}"
                    )
                    phaser_info["AF_cluster_mode"]["success"] = True
                    break
                else:
                    logging.info(
                        f"Final update: Total missing copies: {total_missing_copies}. Total found copies: {total_found_copies}"
                    )

    logging.info(
        "Auto processing [Molecular Replacement] part is done. You can check the log file for details at automated_structure_solvation.log."
    )
    ...
    if refinement_results:
        ...
        logging.info(
            f"Selected model from {best_result.mode} with R-free: {best_result.r_free}"
        )
        ...
        if phaser_results:
            ...
            logging.info(
                f"Using phaser run from {best_phaser_result['mode']} with highest TFZ score: {best_phaser_result['tfz_score']}"
            )
            ...
            autobuild_input_model, successful_phaser_map = (
                molecular_replacement.find_autobuild_inputs(output_root)
            )
            logging.warning(
                "No refinement results found. Using fallback method for autobuild inputs."
            )
    ...
    if resolution < 3.5:
        logging.info(
            "The resolution is better than 3.5, will use phenix.autobuild to build the model."
        )
        ...
        if args.skip_autobuild:
            with open(
                os.path.join(autobuild_folder, "AUTOBUILD_COMMAND.txt"), "w"
            ) as cmd_file:
            ...
            if any(elem is None for elem in phenix_autobuild_cmd):
                raise ValueError(
                    "One or more elements in phenix_autobuild_cmd are None."
                )

            phenix_autobuild_process = subprocess.Popen(phenix_autobuild_cmd)
            autobuild_log_path = os.path.join(
                autobuild_folder, "AutoBuild_run_1_/AutoBuild_run_1_1.log"
            )
            ...
            monitor_autobuild_hanging_thread = threading.Thread(
                target=job_monitor.monitor_and_resolve_hangs,
                args=(autobuild_log_path, phenix_autobuild_process),
            )
            monitor_autobuild_hanging_thread.start()

            logging.info(
                f"Monitoring autobuild hanging thread started for {autobuild_log_path}."
            )

            monitor_autobuild_memory_leaking_thread = threading.Thread(
                target=job_monitor.monitor_and_resolve_memory_leaks,
                args=(autobuild_log_path, phenix_autobuild_process),
            )
            monitor_autobuild_memory_leaking_thread.start()

            logging.info(
                f"Monitoring autobuild memory leaking thread started for {autobuild_log_path}."
            )

            phenix_autobuild_process.wait()
            autobuild_temp_dir = os.path.join(
                autobuild_folder, "AutoBuild_run_1_/TEMP0"
            )
            ...
            cc_input_pdb, cc_input_map_coeffs = utilities.get_autobuild_results_paths(
                autobuild_working_path
            )
            logging.info(f"Overall best pdb [Autobuild]: {cc_input_pdb}")
            logging.info(
                f"Overall best refine map coeffs [Autobuild]: {cc_input_map_coeffs}"
            )
        ...
        if "successful_refinement_folder" in locals():
            ...
            cc_input_pdb, cc_input_map_coeffs = utilities.get_refined_pdb_and_map(
                successful_refinement_folder
            )
            ...
            logging.info(
                "The resolution is worse than 3.5, will use phenix.refine to refine the model."
            )
            r_work, r_free, r_factor_folder, _ = utilities.rfactors_from_phenix_refine(
                autobuild_input_model,
                args.mtz_path,
                refine_output_root,
                nproc=args.nproc,
    ...
    if cc_input_map_coeffs is None:
        cc_input_map_coeffs = successful_phaser_map
    phaser_model_map_cc = utilities.calculate_map_model_correlation(
        cc_input_pdb,
        args.mtz_path,
        cc_input_map_coeffs,
        solvent_content,
    ...
    if args.reference_model is not None or args.reference_map is not None:
        reference_model_map_cc = utilities.calculate_map_model_correlation(
            cc_input_pdb,
            args.mtz_path,
            cc_input_map_coeffs,
            solvent_content,
...
def run():
    # 1. Setup the Parser (Inside the function)
    parser = argparse.ArgumentParser(
        description="AF-guided-MR: Automated Molecular Replacement"
    )

    # Required Arguments
    parser.add_argument("--csv_path", help="Path to sequence CSV file", required=True)
    parser.add_argument("--mtz_path", help="Path to X-ray diffraction data (MTZ)", required=True)

    # Recommended / Optional Arguments
    parser.add_argument("--uniprot_id", help="UniProt IDs (comma separated)", default=None)
    parser.add_argument("--copy_numbers", help="Copy numbers (e.g., 2:2)", default=None)
    parser.add_argument("--nproc", type=int, help="Number of processors", default=8)
    parser.add_argument("--skip_autobuild", action="store_true", help="Skip AutoBuild step")

    # 2. Parse the arguments
    args = parser.parse_args()

    # 3. Execution Logic (Adapted from your original main script)
    print("Initializing Data Manager...")
    try:
        # We use relative imports (from .data_manager import DataManager) at top of file
        dm = DataManager(
            mtz_path=args.mtz_path,
        )

        print("Starting Molecular Replacement process...")
        mr = MolecularReplacement(
            data_manager=dm,
        ...
        # This triggers the core automation
        mr.run_process()

    except Exception as e:
        print(f"\n[ERROR] The process failed with the following message: {e}")
        sys.exit(1)
