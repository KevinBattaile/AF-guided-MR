...
def setup_custom_logger(dir):
    ...
    file_handler = logging.FileHandler(
        os.path.join(dir, "automated_structure_solvation.log")
    )
...
def get_available_cores():
    ...
    while True:
        ...
        if cpu_percent > 80:
            logging.info(
                f"CPU usage at {cpu_percent}%. Waiting 60 seconds before checking again..."
            )
            time.sleep(60)
            continue
...
def get_available_gpu():
    ...
    for device_id in range(device_count):
        ...
        logging.info(
            f"GPU {device_id}: {info.used / (1024**2)} MiB used ({used_memory_fraction * 100:.2f}% used)"
        )

        if info.used <= 1000 * 1024 * 1024:
            logging.info(
                f"GPU {device_id} is available with {info.used / (1024**2)} MiB in use."
            )
            return device_id
...
def calculate_mean_plddt(pdb_file):
    ...
    try:
        with open(pdb_file, "r") as f:
            for line in f:
                if (
                    line.startswith("ATOM") and line[13:16].strip() == "CA"
                ):  # Check if line is for a C-alpha atom
                    try:
                        b_factor = float(
                            line[60:66].strip()
...
def extract_rfactors(sub_folder_path):
    ...
    *and for refinement folders, this sub_folder_path is ./refine/refine_???.
    ...
    def find_refinement_log_file(autobuild_log_path):
        try:
            with open(autobuild_log_path, "r") as file:
                lines = file.read()
                log_refine_path = re.search(
                    r"log_refine: (.+\.log_refine)", lines
                ).group(1)
    ...
    def extract_rfactors_from_refine_log(
        refine_folder_path,
    ):  # folder from phenix.refine
        try:
            refine_log_files = glob.glob(
                os.path.join(refine_folder_path, "*_refine_001.log")
            )
            if refine_log_files:
                with open(refine_log_files[0], "r") as file:
                    ...
                    for line in reversed(lines):
                        if "Final R-work =" in line:
                            r_work, r_free = re.findall(
                                r"Final R-work = ([\d.]+), R-free = ([\d.]+)", line
                            )[0]
                            return float(r_work), float(r_free), True
        except Exception as e:
    ...
    if os.path.exists(autobuild_log_path):
        ...
        if log_refine_path:
            r_work, r_free, extracted = extract_rfactors_from_refinement_log(
                log_refine_path
            )
        if not extracted:
            r_work, r_free, extracted = extract_rfactors_from_autobuild_log(
                autobuild_log_path
            )
...
def rfactors_from_phenix_refine(pdb_path, data_path, refine_output_root, nproc):
    ...
    existing_folders = [
        f
        for f in os.listdir(refine_output_root)
        if os.path.isdir(os.path.join(refine_output_root, f))
    ]
    ...
    def run_phenix_refine(cmd):
        ...
        logging.info(
            f"Running Phenix refine with the following command into {refinement_folder}: {formatted_cmd}"
        )
    ...
    for iteration in range(max_iterations):
        ...
        if (
            ...
            in combined_output
        ) and ("multiple_arrays" not in fixed_errors):
            logging.warning(
                "Multiple equally suitable arrays of observed X-ray data found. Extracting the first possible choice."
            )
            match = re.search(r"Possible choices:\s*(.*)", combined_output, re.DOTALL)
            if match:
                first_choice = match.group(1).split("\n")[0].strip()
                phenix_refine_cmd.append(
                    f"refinement.input.xray_data.labels={first_choice}"
                )
        ...
        if (
            "Atoms at special positions are within rigid groups" in combined_output
        ) and ("atoms_special_positions" not in fixed_errors):
            logging.warning(
                "Atoms at special positions are within rigid groups. Changing strategy to individual_sites+individual_adp and adding --overwrite."
            )
        ...
        if (
            ("R-free flags not compatible" in combined_output)
            or ("missing flag" in combined_output)
        ) and ("r_free_flags" not in fixed_errors):
            logging.warning(
                "R-free flags not compatible or missing flag. Generating new flags with fraction=0.05 and max_free=500."
            )
    ...
    logging.info(
        f"Phenix refine finished for {os.path.join(os.path.basename(refinement_folder), os.path.basename(pdb_path))}."
    )
    ...
    if mtz_files and os.path.exists(mtz_files[0]):
        shutil.move(
            mtz_files[0], os.path.join(refine_output_root, "refinement_data.mtz")
        )
...
def get_autobuild_results_paths(autobuild_working_path):
    ...
    overall_best_refine_map_coeffs = os.path.join(
        autobuild_working_path, "overall_best_refine_map_coeffs.mtz"
    )
...
def calculate_map_model_correlation(
    pdb_file,
    data_file,
    map_file,
    solvent_content,
    ...
    try:
        ...
        try:
            if (
                reference_pdb is None and reference_map is None
            ):  # this is for phaser pdb or autobuild/refinement pdb
                ...
                reference_pdb is not None
            ):  # this is for regular cc calculation using reference pdb
            ...
            logging.error(
                f"An error occurred during map-model correlation calculation: {e}"
            )
        ...
        if (reference_pdb is None and reference_map is not None) or not os.path.exists(
            cc_log_path
        ):
        ...
        # Log the exception if needed
        logging.error(
            f"An error occurred during map-model/map correlation calculation: {e}"
        )
